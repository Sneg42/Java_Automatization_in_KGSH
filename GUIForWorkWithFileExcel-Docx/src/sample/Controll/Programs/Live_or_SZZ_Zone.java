 ...
 */
public class Live_or_SZZ_Zone implements CreateFile{
    ...
    public static XWPFDocument go(XWPFDocument doc, int zone){
        ...
        try{
            // открыть файл эксель и прочитать ячейку
             excel = new XSSFWorkbook("D:\\1\\tbl3_5.xlsx");
            ...
            for (int i = 8; i < 250; i++) {
                ...
                try{
                    if (Double.parseDouble(stringCurrentNumber[0]) > 0.1){
                        ...
                        try{
                            numbEmiss = Integer.parseInt(excel.getSheetAt(0).getRow(i).getCell(0)
                        ...
                        HelpModule.inputInTable(doc, mapEmission.get(numbEmiss), stringCurrentNumber[0]
                                + " ПДК (вклад объекта " +  stringCurrentNumber[1] + " ПДК)");
                    }
        ...
        if (zone == 3) {
            HelpModule.createParag(doc, 0,"По остальным загрязняющим веществам приземные концентрации " +
                    "ниже 0,1 ПДК.");
            HelpModule.createParag(doc, 0,"Перечень источников, дающих наибольшие вклады в уровень загрязнения " +
                    "атмосферы и расчетные максимальные приземные концентрации загрязняющих веществ в жилой зоне и " +
                    "на границе СЗЗ приведены в таблице 5.3-4.");
        }
    ...
    public void createFile(String namePathForFile){
        ...
        // Запись файла
        try{
            FileOutputStream fileDoc = new FileOutputStream(namePathForFile + "\\Расчетн. приземн концентрац.docx");
            doc.write(fileDoc);
            fileDoc.close();
        }catch (Exception e ){
            e.printStackTrace();
